...
async function algorithm(input) {
  //Finds the most likely product to match with a user input by checking matching words, substrings, etc.
        ...
        for (var l = 0; l < search.length; l++) {
          ...
          if (
            search[l]
              .toUpperCase()
              .includes(
                substring(keywords, k, keywords.length + j - 1).toUpperCase()
              )
          ) {
  ...
  for (let i = 0; i < itemNames.length; i++) {
    await page.goto(
      "https://www.amazon.com/s?k=" +
        itemNames[i] +
        "&i=amazonfresh&crid=JV7RYRCN3LM&sprefix=apple%2Camazonfresh%2C157&ref=nb_sb_noss_1"
    );
